...
public class SnapshotCamera : MonoBehaviour
{
    ...
    public static SnapshotCamera MakeSnapshotCamera(int layer = 5, string name = "Snapshot Camera")
    {
        if (layer < 0 || layer > 31)
            throw new ArgumentOutOfRangeException("layer",
                "layer argument must specify a valid layer between 0 and 31");

        // Create a new GameObject to hold the camera
        var snapshotCameraGO = new GameObject(name);
        // Add a Camera component to the GameObject
        var cam = snapshotCameraGO.AddComponent<Camera>();
        ...
        var snapshotCamera = snapshotCameraGO.AddComponent<SnapshotCamera>();
    ...
    /// <summary>
    ///     Takes a snapshot of whatever is in front of the camera and within the camera's culling mask and returns it as a
    ///     Texture2D.
    /// </summary>
    ...
    private Texture2D TakeSnapshot(Color backgroundColor, int width, int height)
    {
        ...
        var texture = new Texture2D(cam.targetTexture.width, cam.targetTexture.height, TextureFormat.ARGB32, false);
...
#pragma warning disable 0649
    /// <summary> /// The Camera used internally by the SnapshotCamera. /// </summary>
    private Camera cam;
...
#pragma warning restore 0649
    ...
    private static string SanitizeFilename(string dirty)
    {
        ...
        var invalidRegStr = string.Format(@"([{0}]*\.+$)|([{0}]+)", invalidFileNameChars);
    ...
    ///     The directory in which to save the file. This will be the game/Snapshots directory if not
    ///     specified.
    /// </param>
    ...
    public static FileInfo SavePNG(byte[] bytes, string filename = "", string directory = "")
    {
        directory = directory != ""
            ...
            : Directory.CreateDirectory(Path.Combine(Application.dataPath, "../Snapshots")).FullName;
        filename = filename != ""
            ? SanitizeFilename(filename) + ".png"
            : DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss-ffff") + ".png";
    ...
    /// <summary>
    ///     This stores the a state (layers, position, rotation, and scale) of a GameObject, and provides a method to restore
    ///     it.
    /// </summary>
    private struct GameObjectStateSnapshot
    {
        ...
        private readonly Dictionary<GameObject, int> layers;
    ...
    /// <summary>
    ///     Set the layers of the GameObject and all its children to the SnapshotCamera's snapshot layer so the SnapshotCamera
    ///     can see it.
    /// </summary>
    ...
    /// <summary>
    ///     Prepares an instantiated GameObject for taking a snapshot by setting its layers and applying the specified position
    ///     offset, rotation, and scale to it.
    /// </summary>
    ...
    /// <returns>
    ///     A GameObjectStateSnapshot containing the state of the gameObject prior to modifying its layers, position,
    ///     rotation, and scale.
    /// </returns>
    private GameObjectStateSnapshot PrepareObject(GameObject gameObject, Vector3 positionOffset, Quaternion rotation,
        Vector3 scale)
    {
        var goss = new GameObjectStateSnapshot(gameObject);
    ...
    /// <summary>
    ///     Prepares a prefab for taking a snapshot by creating an instance, setting its layers and applying the specified
    ///     position offset, rotation, and scale to it.
    /// </summary>
    ...
    {
        var gameObject = Instantiate(prefab, transform.position + positionOffset, rotation);
        gameObject.transform.localScale = scale;
        SetLayersRecursively(gameObject);
    ...
    ///     applies the default position offset, rotation and scale to the gameObject while taking the snapshot, and restores
    ///     them afterwards.
    /// </summary>
    ...
    {
        return TakeObjectSnapshot(gameObject, Color.clear, defaultPositionOffset, Quaternion.Euler(defaultRotation),
    ...
    {
        return TakeObjectSnapshot(gameObject, backgroundColor, defaultPositionOffset, Quaternion.Euler(defaultRotation),
            defaultScale, width, height);
    }
    ...
    ///     The position offset relative to the SnapshotCamera that will be applied to the gameObject
    ///     while taking the snapshot. Its position will be restored after taking the snapshot.
    /// </param>
    /// <param name="rotation">
    ///     The rotation that will be applied to the gameObject while taking the snapshot. Its rotation will
    ///     be restored after taking the snapshot.
    /// </param>
    /// <param name="scale">
    ///     The scale that will be applied to the gameObject while taking the snapshot. Its scale will be
    ///     restored after taking the snapshot.
    /// </param>
    /// <param name="width">The width of the snapshot image.</param>
    /// <param name="height">The height of the snapshot image.</param>
    /// <returns>A Texture2D containing the captured snapshot.</returns>
    public Texture2D TakeObjectSnapshot(GameObject gameObject, Vector3 positionOffset, Quaternion rotation,
    ...
    /// <param name="height">The height of the snapshot image.</param>
    /// <returns>A Texture2D containing the captured snapshot.</returns>
    public Texture2D TakeObjectSnapshot(GameObject gameObject, Color backgroundColor, Vector3 positionOffset,
        Quaternion rotation, Vector3 scale, int width = 128, int height = 128)
    {
        if (gameObject == null)
            throw new ArgumentNullException("gameObject");
        if (gameObject.scene.name == null)
            throw new ArgumentException(
                "gameObject parameter must be an instantiated GameObject! If you want to use a prefab directly, use TakePrefabSnapshot instead.",
                "gameObject");
    ...
    {
        return TakePrefabSnapshot(prefab, Color.clear, defaultPositionOffset, Quaternion.Euler(defaultRotation),
            defaultScale, width, height);
    }
    ...
    {
        return TakePrefabSnapshot(prefab, backgroundColor, defaultPositionOffset, Quaternion.Euler(defaultRotation),
            defaultScale, width, height);
    }
    ...
    /// <param name="height">The height of the snapshot image.</param>
    /// <returns>A Texture2D containing the captured snapshot.</returns>
    public Texture2D TakePrefabSnapshot(GameObject prefab, Color backgroundColor, Vector3 positionOffset,
        Quaternion rotation, Vector3 scale, int width = 128, int height = 128)
    {
        if (prefab == null)
        ...
        if (prefab.scene.name != null)
            throw new ArgumentException(
                "prefab parameter must be a prefab! If you want to use an instance, use TakeObjectSnapshot instead.",
                "prefab");
